    ...
    >
      <div className="max-w-[1600px] mx-auto px-8 lg:px-16">
        <div className="flex items-center justify-between h-24">
          {/* Logo */}
          <Link href="/" className={`group flex items-center space-x-4 ${!true && "-translate-y-100"} duration-500`}>
          ...
          <button
            onClick={() => fetch('/api/auth/agentLogout').then(res => res.json()).then(data => { console.log(data); if (data.message === 'success') router.push('/admin/login') }).catch(err => alert(err))}
