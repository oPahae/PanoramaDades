...
export default function AdminClients() {
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelectedRows(currentClients.map(client => client.id));
    } else {
      setSelectedRows([]);
    }
  };
  ...
  const handleDownloadExcel = async () => {
    if (selectedRows.length === 0) {
      alert('Please select at least one client to download');
      return;
    }
    ...
    try {
      ...
      const response = await fetch('/api/excel/customers', {
        ...
        body: JSON.stringify({ customers: selectedClients }),
      });
  ...
  const isAllSelected = currentClients.length > 0 && currentClients.every(client => selectedRows.includes(client.id));
  const isSomeSelected = currentClients.some(client => selectedRows.includes(client.id)) && !isAllSelected;
              ...
              <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                <div className="text-sm text-stone-600">
                  {selectedRows.length > 0 ? (
                    ...
                    <span>Select clients to download</span>
                  )}
                </div>
                <button
