...
export default function AddRoom() {
  ...
  const [USDR, setUSDR] = useState(0);
  const [CHFR, setCHFR] = useState(0);

  async function getRate(to) {
    try {
      const res = await fetch('https://open.er-api.com/v6/latest/MAD');
      const data = await res.json();
      if (data.result !== 'success') throw new Error(data['error-type']);
      return to === 'USD' ? data.rates.USD : data.rates.CHF;
    } catch (err) {
                ...
                <div className="space-y-4">
                  <div className="flex justify-center">
                    <div className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-all ${isDragging
                      ...
                      <Upload className={`w-8 h-8 transition-colors ${isDragging
                        ? 'text-amber-600'
                        : 'text-stone-400'
                        }`} />
                    </div>
                  ...
                  <div>
                    <p className={`text-lg font-medium mb-2 transition-colors ${isDragging
                      ? 'text-amber-900'
                      : 'text-stone-700'
                      }`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="space-y-3">
                  ...
                  <input
                    ...
                    onChange={(e) => { handleInputChange(e); setFormData({ ...formData, priceMAD: e.target.value, priceUSD: (Number(e.target.value) * USDR).toFixed(2), priceCHF: (Number(e.target.value) * CHFR).toFixed(2) }) }}
                    required
                    min="0"
                    step="any"
                    className="w-full px-4 py-4 bg-stone-50 border border-stone-200 rounded-lg text-stone-900 placeholder-stone-400 focus:border-amber-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-amber-900/10 transition-all"
