...
// Import ApexCharts dynamiquement pour éviter les problèmes SSR
...
export default function Home() {
  ...
  // Colors
  const ROOM_COLORS = ['#10b981', '#f59e0b', '#ef4444'];
  const RESERVATION_COLORS = ['#3b82f6', '#ef4444', '#10b981', '#6b7280'];
  const INVOICE_COLORS = ['#f59e0b', '#10b981', '#ef4444'];

  // Chart Options
  const getRoomStatusChartOptions = () => ({
    chart: {
      type: 'pie',
      fontFamily: 'inherit',
    },
    labels: stats?.rooms?.statusDistribution?.map(item => item.name) || [],
    colors: ROOM_COLORS,
    legend: {
      position: 'bottom',
      fontSize: '14px',
  ...
  const getReservationsRevenueChartOptions = () => ({
    chart: {
      type: 'bar',
      fontFamily: 'inherit',
      toolbar: {
    ...
    xaxis: {
      categories: stats?.rooms?.byRoom?.map(item => item.roomTitle) || [],
      labels: {
        rotate: -45,
        style: {
          fontSize: '12px'
  ...
  const getRevenueOverTimeChartOptions = () => ({
    chart: {
      type: 'line',
      fontFamily: 'inherit',
      toolbar: {
    ...
    xaxis: {
      categories: stats?.financial?.revenueOverTime?.map(item => item.date) || [],
      labels: {
        rotate: -45,
        style: {
          fontSize: '12px'
  ...
  const getReservationsByStatusChartOptions = () => ({
    chart: {
      type: 'pie',
      fontFamily: 'inherit',
    },
    labels: stats?.financial?.reservationsByStatus?.map(item => item.name) || [],
    colors: RESERVATION_COLORS,
    legend: {
      position: 'bottom',
      fontSize: '12px',
    },
  ...
  const getInvoicesByStatusChartOptions = () => ({
    chart: {
      type: 'pie',
      fontFamily: 'inherit',
    },
    labels: stats?.financial?.invoicesByStatus?.map(item => item.name) || [],
          ...
          <div className="flex flex-col gap-6 mb-12">
            <div className="space-y-4">
              ...
              <h1 className="text-2xl md:text-4xl font-serif font-bold text-stone-900 leading-tight">
                Analytics
                <span className="ml-2 italic text-amber-900">Overview</span>
              </h1>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-lg border border-stone-200/50 p-4 sm:p-8">
              ...
              <div className="w-full">
                <Chart
                  options={getRoomStatusChartOptions()}
                  series={stats?.rooms?.statusDistribution?.map(item => item.value) || []}
                  type="pie"
                  height={350}
                />
              </div>
            ...
            <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-lg border border-stone-200/50 p-4 sm:p-8">
              ...
              <div className="w-full">
                <Chart
                  options={getReservationsRevenueChartOptions()}
                  series={[
                    {
                      name: 'Reservations',
                      data: stats?.rooms?.byRoom?.map(item => item.reservations) || []
                    },
                    {
                      name: 'Revenue (MAD)',
                      data: stats?.rooms?.byRoom?.map(item => item.revenue) || []
                    }
                  ]}
                  type="bar"
                  height={350}
        ...
        <div className="max-w-[1600px] mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 xl:col-span-2 bg-white/95 backdrop-blur-xl rounded-2xl shadow-lg border border-stone-200/50 p-4 sm:p-8">
              ...
              <div className="w-full">
                <Chart
                  options={getRevenueOverTimeChartOptions()}
                  series={[
                    {
                      name: 'Revenue (MAD)',
                      data: stats?.financial?.revenueOverTime?.map(item => item.revenue) || []
                    }
                  ]}
                  type="line"
                  height={350}
            ...
            <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-lg border border-stone-200/50 p-4 sm:p-8">
              ...
              <div className="w-full">
                <Chart
                  options={getReservationsByStatusChartOptions()}
                  series={stats?.financial?.reservationsByStatus?.map(item => item.value) || []}
                  type="pie"
                  height={340}
                />
              </div>
            ...
            <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-lg border border-stone-200/50 p-4 sm:p-8">
              ...
              <div className="w-full">
                <Chart
                  options={getInvoicesByStatusChartOptions()}
                  series={stats?.financial?.invoicesByStatus?.map(item => item.value) || []}
                  type="pie"
                  height={320}
                />
              </div>
                    ...
                    <tr key={customer.id} className="hover:bg-amber-50/30 transition-colors">
                      ...
                      <td className="px-4 sm:px-6 py-4">
                        <div className="text-sm font-semibold text-green-900">
                          {customer.totalPaid.toFixed(2)} MAD
                        </div>
                      </td>
                      <td className="px-4 sm:px-6 py-4">
                        <div className="text-sm font-semibold text-amber-900">
                          {customer.totalPending.toFixed(2)} MAD
                        </div>
                      </td>
                    </tr>
                  ))}
