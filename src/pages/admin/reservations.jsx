...
export default function AdminReservations() {
  ...
  useEffect(() => {
    ...
    if (formData.roomID) {
      ...
      if (selectedRoom) {
        setFormData(prev => ({
          ...prev,
          amount: selectedRoom.priceMAD * nights
        }));
      }
    }
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelectedRows(currentReservations.map(reservation => reservation.id));
    } else {
      setSelectedRows([]);
    }
  };

  const handleSelectRow = (reservationId) => {
  ...
  const handleDownloadExcel = async () => {
    if (selectedRows.length === 0) {
      alert('Please select at least one reservation to download');
      return;
    }
  ...
  const getRowStyle = (status, isSelected) => {
    ...
    if (isSelected) {
      return `${baseStyle} bg-amber-50/50`;
    }
    switch (status) {
      case 'finished':
        return `${baseStyle} bg-green-50/30 hover:bg-green-50/50`;
      case 'canceled':
        return `${baseStyle} opacity-50 line-through hover:bg-gray-50/50`;
      default:
        return `${baseStyle} hover:bg-amber-50/30`;
    }
  };
  ...
  const isAllSelected = currentReservations.length > 0 && currentReservations.every(reservation => selectedRows.includes(reservation.id));
  const isSomeSelected = currentReservations.some(reservation => selectedRows.includes(reservation.id)) && !isAllSelected;
        ...
        <div className="max-w-[1600px] mx-auto">
          {loading ? (
            ...
            <div className="space-y-6">
              {/* Download Button and Selection Info */}
              <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                <div className="text-sm text-stone-600">
                  {selectedRows.length > 0 ? (
                    <span className="font-semibold text-amber-900">
                      {selectedRows.length} reservation{selectedRows.length !== 1 ? 's' : ''} selected
                    </span>
                  ) : (
                    <span>Select reservations to download</span>
                  )}
                </div>
                <button
                  ...
                  <span className="relative z-10 text-white flex items-center space-x-2">
                    {downloading ? (
                      ...
                      <>
                        ...
                        <span>Download Excel ({selectedRows.length})</span>
                      </>
                    )}
                  </span>
                </button>
                  ...
                  <table className="w-full">
                      ...
                      <tr className="bg-gradient-to-r from-amber-50 to-amber-100/50 border-b-2 border-amber-200">
                        <th className="px-6 py-4 text-left">
                          <input
                            ...
                            className="w-5 h-5 text-amber-900 border-2 border-stone-300 rounded focus:ring-2 focus:ring-amber-900 focus:ring-offset-0 cursor-pointer"
                          />
                        </th>
                    ...
                    <tbody className="divide-y divide-stone-200">
                      {currentReservations.map((reservation) => {
                        ...
                        const isSelected = selectedRows.includes(reservation.id);
                        return (
                          <tr
                            key={reservation.id}
                            className={getRowStyle(reservation.status, isSelected)}
                          >
                            ...
                            <td className="px-6 py-4">
                              <span className={`inline-flex gap-1 items-center px-3 py-1 rounded-full text-xs font-semibold border ${getStatusStyle(reservation.status)}`}>
                                {reservation.status === 'finished' && <CheckCheck size={16} />}
                                {reservation.status}
                              </span>
                            </td>
                            <td className="px-6 py-4">
                    ...
                    formatOptionLabel={(option) => {
                      ...
                      return (
                        <div className="flex items-center gap-2">
                          <TypeIcon
                            className={`w-5 h-5 ${option.type === 'agency' ? 'text-blue-600' : 'text-green-600'
                              }`}
                          />
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setFormData({ ...formData, roomID: selectedRoom?.id, amount: selectedRoom?.priceMAD * nbrNights });
                    }}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) => setFormData({ ...formData, discount: e.target.value > 100 ? 100 : e.target.value })}
                      className="w-full pl-12 pr-4 py-3 border-2 border-stone-200 rounded-lg focus:border-amber-900 focus:outline-none transition-colors"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      max="100"
                      value={formData.tva}
                      onChange={(e) => setFormData({ ...formData, tva: e.target.value > 100 ? 100 : e.target.value })}
                      className="w-full pl-12 pr-4 py-3 border-2 border-stone-200 rounded-lg focus:border-amber-900 focus:outline-none transition-colors"
                    />
                  </div>
                </div>
