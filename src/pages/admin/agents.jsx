...
export default function AdminAgents() {
  ...
  const handleDownloadExcel = async () => {
    if (selectedRows.length === 0) {
      alert('Please select at least one agent to download');
      return;
    }

    try {
      setDownloading(true);
      const selectedAgents = filteredAgents.filter(agent => 
  ...
  const isAllSelected = currentAgents.length > 0 && currentAgents.every(agent => selectedRows.includes(agent.id));
  const isSomeSelected = currentAgents.some(agent => selectedRows.includes(agent.id)) && !isAllSelected;
              ...
              <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                <div className="text-sm text-stone-600">
                  {selectedRows.length > 0 ? (
                    ...
                    <span>Select agents to download</span>
                  )}
                </div>
                <button
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-stone-200">
                      {currentAgents.map((agent) => {
                        const isSelected = selectedRows.includes(agent.id);
                        return (
                          <tr
                            key={agent.id}
                            className={`transition-colors duration-150 ${
                              isSelected ? 'bg-amber-50/50' : 'hover:bg-amber-50/30'
                            }`}
                          >
